...
int main(int argc, char *argv[] ) 
 { 
    ...
    std::chrono::time_point<std::chrono::system_clock> start, end;
    std::vector<int> pixels(W*H);
    std::vector<int> localPixels(W*H/nbp);

    start = std::chrono::system_clock::now();

    MPI_Scatter(pixels.data(), W*H/nbp, MPI_INT, localPixels.data(), W*H/nbp, MPI_INT, 0, MPI_COMM_WORLD);

    for ( int i = 0; i < H/nbp; ++i ) {
        computeMandelbrotSetRow(W, H, maxIter, i, localPixels.data() + W*(H-i-1) );
    }

    MPI_Gather(localPixels.data(), W*H/nbp, MPI_INT, pixels.data(), W*H/nbp, MPI_INT, 0, MPI_COMM_WORLD);
