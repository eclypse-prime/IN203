...
std::vector<int>
computeMandelbrotSet( int W, int H, int maxIter, int rank, int nbp)
{
    std::chrono::time_point<std::chrono::system_clock> start, end;
    std::vector<int> pixels(W*H);
    start = std::chrono::system_clock::now();
    // On parcourt les pixels de l'espace image :
    for (int i = (nbp-rank-1)*H/nbp; i < (nbp-rank)*H/nbp; ++i ) {
      computeMandelbrotSetRow(W, H, maxIter, i, pixels.data() + W*(H-i-1) );
    }
    end = std::chrono::system_clock::now();
    std::chrono::duration<double> elapsed_seconds = end-start;
    std::cout << "Temps calcul processus " << rank << " : " << elapsed_seconds.count() 
              << std::endl;
    return pixels;
}
...
//Démarrer le chronomètre
    ...
    if (rank == 0) { //MASTER
        ...
        while (count2 < H) {
            for(int i = 1; i<nbp; i++) {
                MPI_Send(&count1 , 1, MPI_INT , i, 0, MPI_COMM_WORLD);
                count1++;
                if (count1 == H) break;
            }
            for(int i = 1; i<nbp; i++) {
                MPI_Recv(image.data()+W*count2, W, MPI_INT, i, 1, MPI_COMM_WORLD, &status);
                count2++;
                if (count2 == H) break;
            }
        }
        ...
        while(1) {
            MPI_Recv(&ligne, 1, MPI_INT, 0, 0, MPI_COMM_WORLD, &status);
            if(ligne == -1) break;
            computeMandelbrotSetRow(W, H, maxIter, ligne, image.data() + W*(H-ligne-1) );
            MPI_Send(image.data() + W*(H-ligne-1), W, MPI_INT, 0, 1, MPI_COMM_WORLD);
        }
    }

    // //Calculer une partie de l'image attribuée au processus
    // auto iters = computeMandelbrotSet(W, H, maxIter, rank, nbp);

    // //Rassembler les résultats dans le vecteur image
    // std::vector<int> image(W*H);
    // MPI_Gather(iters.data()+(rank*W*H/nbp), W*H/nbp, MPI_INT, image.data(), W*H/nbp, MPI_INT, 0, MPI_COMM_WORLD);

    //Arrêter le chronomètre et afficher le résultat
    if (rank == 0) {
        end = std::chrono::system_clock::now();
        std::chrono::duration<double> elapsed_seconds = end-start;
        std::cout << "Temps calcul total : " << elapsed_seconds.count() << std::endl;
    }

    //Sauvegarder le résultat
    if (rank == 0) {
        savePicture("mandelbrot.ppm", W, H, image, maxIter);
    }
