...
int main(int nargs, char* argv[])
{
	...
	//Commandes MPI pour récupérer nbp, rank
    ...
    for ( std::int64_t num_config = 0; num_config < (1LL<<nb_cases); ++num_config)
    {
        ...
        std::vector<std::uint8_t> cells(nb_iter*nb_cells/nbp, 0);
        
        //cells[nb_cells/2] = 1;
        if (rank * nb_cells/nbp <= nb_cells/2 && nb_cells/2 < (rank+1) * nb_cells/nbp) {
			cells[nb_cells/2 - rank*nb_cells/nbp] = 1;
		}
